...
module.exports = {
  ...
  loadMenuBar: function(win) {
    ...
    menu.createMacBuiltin('Hangouts');
  ...
  createTrayMenu: function(win) {
    ...
    menu.append(new gui.MenuItem({
      ...
      click: function() {
        win.show();
        win.open('https://hangouts.google.com')
      }
    }));
  ...
  loadTrayIcon: function(win) {
    ...
    tray.tooltip = 'Hangouts for Desktop';
  ...
  createContextMenu: function(win, window, document, targetElement) {
    ...
    if (targetElement.tagName.toLowerCase() == 'input') {
      ...
      menu.append(new gui.MenuItem({
        label: "Copy Link",
        click: function() {
          var url = utils.skipHangoutsRedirect(targetElement.href);
          clipboard.set(url);
        }
      }));
    } else {
