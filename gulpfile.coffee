    ...
    gulp.src './src/**'
      .pipe $.nodeWebkitBuilder
        ...
        macPlist:
          ...
          CFBundleIdentifier: 'com.pierrejacquier.hangoutsfordesktop'
      .on 'end', ->
        if process.argv.indexOf('--toolbar') > 0
  ...
  gulp.task 'build:' + platform, ->
    ...
    gulp.src './src/**'
      .pipe $.nodeWebkitBuilder
        ...
        macPlist:
          NSHumanReadableCopyright: 'pierrejacquier.com'
          CFBundleIdentifier: 'com.aluxian.hangoutsfordesktop'
      .on 'end', ->
        if process.argv.indexOf('--toolbar') > 0
          shelljs.sed '-i', '"toolbar": true', '"toolbar": false', './src/package.json'
...
gulp.task 'sign:osx64', ['build:osx64'], ->
  shelljs.exec 'codesign -v -f -s "Pierre Jacquier Apps" ./build/Hangouts/osx64/Hangouts.app/Contents/Frameworks/*'
  shelljs.exec 'codesign -v -f -s "Pierre Jacquier Apps" ./build/Hangouts/osx64/Hangouts.app'
  shelljs.exec 'codesign -v --display ./build/Hangouts/osx64/Hangouts.app'
  shelljs.exec 'codesign -v --verify ./build/Hangouts/osx64/Hangouts.app'
...
gulp.task 'pack:osx64', ['sign:osx64'], ->
  ...
  shelljs.rm '-f', './dist/Hangouts.dmg' # appdmg fails if the dmg already exists

  gulp.src []
    .pipe require('gulp-appdmg')
      source: './assets-osx/dmg.json'
      target: './dist/Hangouts.dmg'
...
[32, 64].forEach (arch) ->
  ['deb', 'rpm'].forEach (target) ->
    gulp.task "pack:linux#{arch}:#{target}", ['build:linux' + arch], ->
      ...
      move_opt = gulp.src [
        './assets-linux/hangoutsfordesktop.desktop'
        './assets-linux/after-install.sh'
        './assets-linux/after-remove.sh'
        './build/Hangouts/linux' + arch + '/**'
      ]
        .pipe gulp.dest './build/linux/opt/HangoutsForDesktop'

      move_png48 = gulp.src './assets-linux/icons/48/hangoutsfordesktop.png'
        .pipe gulp.dest './build/linux/usr/share/icons/hicolor/48x48/apps'

      move_png256 = gulp.src './assets-linux/icons/256/hangoutsfordesktop.png'
        .pipe gulp.dest './build/linux/usr/share/icons/hicolor/256x256/apps'

      move_svg = gulp.src './assets-linux/icons/scalable/hangoutsfordesktop.png'
      ...
      mergeStream move_opt, move_png48, move_png256, move_svg
        .on 'end', ->
          ...
          output = "../../dist/Hangouts_linux#{arch}.#{target}"

          shelljs.mkdir '-p', '../../dist' # it fails if the dir doesn't exist
          shelljs.rm '-f', output # it fails if the package already exists

          shelljs.exec "fpm -s dir -t #{target} -a #{port} -n hangoutsfordesktop --after-install ./opt/HangoutsForDesktop/after-install.sh --after-remove ./opt/HangoutsForDesktop/after-remove.sh --license MIT --category Chat --url \"https://pierrejacquier.com/projects/hangoutsfordesktop.html\" --description \"A simple and beautiful app for Google Hangouts.\" -m \"Pierre Jacquier <pierrejacquier39@gmail.com>\" -p #{output} -v #{manifest.version} ."
          shelljs.cd '../..'
...
gulp.task 'run:osx64', ['build:osx64'], ->
  shelljs.exec 'open ./build/Hangouts/osx64/Hangouts.app'
...
gulp.task 'release', ['pack:linux64:deb'], (callback) ->
  gulp.src './dist/*'
    .pipe $.githubRelease
      draft: true
      manifest: manifest
